...
keywords:
    ...
    - 逆向
    - 爬蟲
description: js逆向 某空氣質量網
cover: Untitled.png
---

> 網址：[http://www.aqistudy.cn/historydata/monthdata.php?city=北京](http://www.aqistudy.cn/historydata/monthdata.php?city=%E5%8C%97%E4%BA%AC)
> 

## 反爬手段

1. F12檢測：用`ctrl+shift+i`，然後將開發者工具彈出
    
    ![Untitled](Untitled.png)
    
2. 無限Debugger：用油猴注入以下hook腳本
    
    ```jsx
    // ==UserScript==
    // @name         debugger
    // @namespace    http://www.aqistudy.cn/historydata/monthdata.php?city=%E5%8C%97%E4%BA%AC
    // @version      0.1
    // @description  try to take over the world!
    // @author       You
    // @match        http://www.aqistudy.cn/historydata/monthdata.php?city=%E5%8C%97%E4%BA%AC
...
- 一開始在Network面板抓包時，沒有看到ajax請求( 但後來得知，只有在沒有網站緩存時會發送ajax請求來獲取數據 )
- 之後查看Sources中的js代碼，也沒有找到想要的東西
- 最後在網頁源代碼中，看到以下被加密的代碼
...
- 接下來就要想辦法查看`eval`中的內容，我使用的辦法是hook `eval`這個函數
    
    ```jsx
    // ==UserScript==
    // @name         test
    // @namespace    https://ngiokweng.github.io/
    // @version      0.1
    ...
    (function() {
        ...
        function hook(object,attr){
            ...
            object[attr] = function(){
                ...
                debugger; //在這斷下，方便在調試時通過調用棧找到是誰調用該函數
                var ret = func.apply(object,arguments); //調用原本的那個函數，獲取返回值
                return ret;
            }
        }
        hook(window,'eval');
...
- 找到對應的那個`eval`函數，單步步入

![Untitled](Untitled2.png)

### 分析eval函數

- 單步步入`eval`函數後，就能看到"解密"後的內容，分析後發現`suhI9pzg8akH6`是我的目標，繼續跟進去

![Untitled](Untitled3.png)

- `const dMqYT = gFEd9jX6phJLhqad(kQ8z, pw3c5ux)`這句是從`localStorageUtil`中獲取數據，相當於從網站緩存中獲取，因此只有第1次訪問該網站或刪除網站緩存後，才會發送ajax請求來獲取數據

![Untitled](Untitled4.png)

- 先刪除網站緩存：開發者工具→應該程式→儲存空間→清除
- 然後刷新頁面，重新去到發送ajax的地方

![Untitled](Untitled5.png)

### 加密函數分析

- 之後對加密函數進行分析，先看加密參數是怎樣來的

![Untitled](Untitled6.png)

- 經多次測試後發現，下圖兩處地方是會改變的
- 第1個地方：每次隨機生成不同的字符串
- 第2個地方分3種情況：
    1. 只有BASE64
    2. BASE64+AES
    3. BASE64+DES
- 因此寫腳本要注意分3種情況，**並且每次AES、DES加密所用到key和iv都會變，所以要動態獲取**

![Untitled](Untitled7.png)

### 解密函數分析

- 請求成功後，用`dobXiBdA52b`函數對返回的結果進行解密，解密後的數據就是我想要的空氣質量數據

![Untitled](Untitled8.png)

- 可以看到該函數就是簡單的BASE64、DES、AES解密，只需找到key和iv就能輕易地解密
- 且經過多次測試發現該函數的形式不會改變，**只有key和iv會變**
- 因此也需要動態地獲取key和iv
...
1. 因為所有的key和iv都會變，因此需要動態獲取js代碼
2. 而js被加密，所以要對js代碼進行解密
3. 獲取AES、DES的key和iv( 在解密後的js代碼最上方 )
4. 為加密函數、解密函數匹配對應的key、iv ( 加密、解密所用到的key、iv不同，要進行匹配 )
5. 模擬加密函數進行加密
6. 模擬解密函數進行解密

### 完整代碼

- **說明**：`main.py`是主要的python代碼，`main.js`和`func.js`是用來模擬加密、解密過程的代碼，`myHttp.js`提供了幾個api接口，讓`main.py`能執行`main.js`中的js代碼
- 使用步驟：
...
// 以下兩行是為了解決->https://stackoverflow.com/questions/19917401/error-request-entity-too-large
